...
const { passwordResetLimiter, loginLimiter } = require('../middleware/rateLimitMiddleware');
...
describe('Rate Limit Middleware', () => {
    ...
    it('should handle rate limiting for password reset requests', async () => {
        const user = await User.create({ username: 'ratelimituser', email: 'ratelimit@example.com', password: 'password123' });
