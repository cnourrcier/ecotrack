...
describe('Authentication Controller', () => {
    ...
    describe('POST /api/refresh-token', () => {
        it('should refresh the access token with a valid refresh token', async () => {
            const user = await User.create({ username: 'refreshuser', email: 'refresh@example.com', password: 'password123' });
    ...
    describe('POST /api/reset-password-request', () => {
        ...
        it('should set resetPasswordToken and resetPasswordExpires when requesting password reset', async () => {
            const user = await User.create({ username: 'resetuser', email: 'reset@example.com', password: 'password123' });
            const res = await request(app)
                .post('/api/reset-password-request')
                .send({
                    email: 'reset@example.com'
        ...
        it('should update resetPasswordToken and resetPasswordExpires on multiple reset requests for the same user', async () => {
            ...
            expect(res1.body).toHaveProperty('message', 'Password reset email sent');
            ...
            expect(res2.body).toHaveProperty('message', 'Password reset email sent');
            ...
            expect(secondExpires.getTime()).toBeGreaterThan(firstExpires.getTime());
    ...
    describe('POST /api/reset-password-confirm', () => {
        ...
        it('should reset password with valid token', async () => {
            ...
            const updatedUser = await User.findById(user._id);
            expect(await updatedUser.isValidPassword('newpassword123')).toBe(true);
        ...
        it('should handle errors gracefully', async () => {
            ...
            expect(res.statusCode).toBe(500);
            expect(res.body).toHaveProperty('message', 'Error in password reset confirmation');
        });
    });
});